...
let DIFFERENT_VERSIONS = ['vega-lite', 'vega', 'vega-embed'];
let SKIP_CSS = {
...
function ensureJupyterlab() {
    ...
    utils.getLernaPaths().forEach(pkgPath => {
        ...
        let relativePath = utils.ensureUnixPathSep(path.join('..', path.relative(basePath, pkgPath)));
...
async function ensureIntegrity() {
    ...
    for (let name in locals) {
        ...
        let pkgMessages = await ensure_package_1.ensurePackage(options);
        if (pkgMessages.length > 0) {
