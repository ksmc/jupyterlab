...
import { ISettingRegistry, IStateDB, PageConfig, URLExt } from '@jupyterlab/coreutils';
...
import { NotebookTools, INotebookTools, INotebookTracker, NotebookActions, NotebookModelFactory, NotebookPanel, NotebookTracker, NotebookWidgetFactory, StaticNotebook, CommandEditStatus, NotebookTrustStatus } from '@jupyterlab/notebook';
...
export const commandEditItem = {
    ...
    activate: (app, statusBar, tracker) => {
        const { shell } = app;
 ...
 */
function activateNotebookTools(app, tracker, editorServices, state, labShell) {
...
function activateNotebookHandler(app, contentFactory, docManager, editorServices, rendermime, palette, browserFactory, launcher, restorer, mainMenu, settingRegistry) {
    ...
    function updateConfig(settings) {
        let code = Object.assign({}, StaticNotebook.defaultEditorConfig.code, settings.get('codeCellConfig').composite);
        let markdown = Object.assign({}, StaticNotebook.defaultEditorConfig.markdown, settings.get('markdownCellConfig').composite);
        let raw = Object.assign({}, StaticNotebook.defaultEditorConfig.raw, settings.get('rawCellConfig').composite);
 ...
 */
function addCommands(app, docManager, services, tracker, clonedOutputs) {
    ...
    commands.addCommand(CommandIDs.createOutputView, {
        ...
        execute: async (args) => {
            ...
            if (path && index !== undefined && index !== null) {
                ...
                current = getCurrent(Object.assign({}, args, { activate: false }));
                if (!current) {
                    return;
                }
...
function populateMenus(app, mainMenu, tracker, services, palette) {
    ...
    mainMenu.runMenu.codeRunners.add({
        ...
        run: current => {
            const { context, content } = current;
            return NotebookActions.runAndAdvance(content, context.session).then(() => void 0);
        },
        runAll: current => {
            const { context, content } = current;
            return NotebookActions.runAll(content, context.session).then(() => void 0);
        },
