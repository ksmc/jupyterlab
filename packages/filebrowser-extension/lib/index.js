...
export const fileUploadStatus = {
    ...
    activate: (app, statusBar, browser) => {
...
function addCommands(app, factory, labShell, docManager, settingRegistry, commandPalette, mainMenu) {
    ...
    class OpenWithMenu extends Menu {
        onBeforeAttach(msg) {
            ...
            let factories = OpenWithMenu._intersection(map(tracker.currentWidget.selectedItems(), i => {
...
(function (Private) {
    ...
    function getBrowserForPath(path, factory) {
        ...
        if (driveName) {
            let browserForPath = tracker.find(_path => _path.model.driveName === driveName);
            if (!browserForPath) {
